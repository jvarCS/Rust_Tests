    ...
    while !done {
        ...
        match choice {
            1 => {
                ...
                employees.insert(eID, Rc::clone(&temp));
                employees2.insert(eID, temp2);
                // mem::drop(temp); // - { ref count is currently 3. If this uncomments, ref count becomes 2 }
                // Dropping alone without first cloning on the insert results in error and cant compile
