        ...
        match choice {
            ...
            4 => {
                ...
                if let Some(emp_rc) = employees.get(&eId) { 
                    ...
                    // CHANGE HERE - { Will create a separate borrow instance that borrows read and write }
                    let mut emp_rw = emp_rc.borrow_mut();
                    let test_name: String = String::from("Raven");
                    emp_rw.changeName(test_name);
                    // This line compiles but produces runtime error "already borrowed"
                    println!("Name after changing with RW instance: {}", emp.getName());
                }
                else {
